...
namespace basecross {
	...
	void WallSprite::OnCreate() {
		...
		m_DrawContext->CreateDefault2DPipelineCmdList<VertexPositionColorTexture, VSPCTSprite, PSPCTSprite>();
		...
		//初期値更新
		m_DrawContext->UpdateConstantBuffer(reinterpret_cast<void**>(&m_SpriteConstantBuffer), sizeof(m_SpriteConstantBuffer));
		//テクスチャ設定
	...
	void WallSprite::OnDraw() {
		...
		//描画
		m_DrawContext->DrawIndexed<VertexPositionColorTexture>(m_SquareMesh);
