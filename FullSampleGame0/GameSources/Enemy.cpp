...
//		PtrColl->SetDrawActive(true);
	...
	void MozoStart::Execute(const shared_ptr<MozoEnemy>& Obj) {
		...
		if (Vector2EX::Length(PlayerVec2 - Vec2) <= 1.0f) {
			auto PtrGrav = Obj->GetComponent<Gravity>();
	...
	//--------------------------------------------------------------------------------------
	//構築と破棄
	FixedHead::FixedHead(const shared_ptr<Stage>& StagePtr, const Vector3& StartPos) :
		GameObject(StagePtr),
		m_Position(StartPos)
	{
	}
	...
	void FixedCylinder::BalloonObOff() {
		auto PtrTransform = GetComponent<Transform>();
		auto PtrBalloon = GetStage()->GetSharedGameObject<PopupBalloon>(L"PopupBalloon");
		if (m_BalloonActive) {
			PtrBalloon->PopupClose();
			m_BalloonActive = false;
		}
		else {
