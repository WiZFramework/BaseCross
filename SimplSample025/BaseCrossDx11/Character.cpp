...
namespace basecross {
	...
	void SquareObject::OnDraw() {
		...
		auto shptr = PtrGameStage->FindTagGameObject<BcPNTnTStaticRenderer>(L"BcPNTnTStaticRenderer");
	...
	void SimpleSquare::OnDraw() {
		...
		auto PtrGameStage = GetStage<GameStage>();
		auto shptr = PtrGameStage->FindTagGameObject<SimplePCTStaticRenderer>(L"SimplePCTStaticRenderer");
		m_PtrObj->m_WorldMatrix = World;
		shptr->AddDrawObject(m_PtrObj);
...
//		body.m_IsDrawActive = true;
	...
	void StaticChara::OnDrawShadowmap() {
		auto PtrGameStage = GetStage<GameStage>();
		auto shptr = PtrGameStage->FindTagGameObject<ShadowmapRenderer>(L"ShadowmapRenderer");
		shptr->AddDrawObject(m_PtrShadowmapObj);
	}

	void StaticChara::OnDraw() {
		auto PtrGameStage = GetStage<GameStage>();
		auto shptr = PtrGameStage->FindTagGameObject<SimplePNTStaticModelRenderer2>(L"SimplePNTStaticModelRenderer2");
		shptr->AddDrawObject(m_PtrObj);
	}
...
//		body.m_IsDrawActive = true;
	...
	void BoneChara::OnDraw() {
		auto PtrGameStage = GetStage<GameStage>();
		auto shptr = PtrGameStage->FindTagGameObject<SimplePNTBoneModelRenderer2>(L"SimplePNTBoneModelRenderer2");
		shptr->AddDrawObject(m_PtrObj);
	}
...
//		body.m_IsDrawActive = true;
	...
	void SimpleSphereObject::OnDraw() {
		auto PtrGameStage = GetStage<GameStage>();
		auto shptr = PtrGameStage->FindTagGameObject<SimplePNTStaticRenderer2>(L"SimplePNTStaticRenderer2");
		shptr->AddDrawObject(m_PtrObj);
	}
...
//		body.m_IsDrawActive = true;
	...
	void BcSphereObject::OnDraw() {
		auto PtrGameStage = GetStage<GameStage>();
		auto shptr = PtrGameStage->FindTagGameObject<BcPNTStaticRenderer>(L"BcPNTStaticRenderer");
		shptr->AddDrawObject(m_PtrObj);
	}
	...
	//static変数の初期化
	shared_ptr<MeshResource> BcBoxObject::m_BoxMesh(nullptr);

	void BcBoxObject::CreateBuffers() {
		if (!m_BoxMesh) {
			//まだ頂点が作成されてないときに作成
			vector<VertexPositionNormalTexture> vertices;
			vector<VertexPositionNormalTangentTexture> new_vertices;
			...
			BcRenderer::ConvertToNormalVertex(vertices, new_vertices);
			//メッシュの作成（変更できない）
			m_BoxMesh = MeshResource::CreateMeshResource(new_vertices, indices, false);
		}
	}
...
//		body.m_IsDrawActive = true;
	...
	void BcStaticChara::OnDraw() {
		auto PtrGameStage = GetStage<GameStage>();
		auto shptr = PtrGameStage->FindTagGameObject<BcPNTStaticModelRenderer>(L"BcPNTStaticModelRenderer");
		shptr->AddDrawObject(m_PtrObj);
	}
...
//		body.m_IsDrawActive = true;
	...
	void BcStaticNormalChara::OnDraw() {
		auto PtrGameStage = GetStage<GameStage>();
		auto shptr = PtrGameStage->FindTagGameObject<BcPNTnTStaticModelRenderer>(L"BcPNTnTStaticModelRenderer");
		shptr->AddDrawObject(m_PtrObj);
	}
...
//		body.m_IsDrawActive = true;
	...
	void BcBoneChara::OnDraw() {
		auto PtrGameStage = GetStage<GameStage>();
		auto shptr = PtrGameStage->FindTagGameObject<BcPNTBoneModelRenderer>(L"BcPNTBoneModelRenderer");
		shptr->AddDrawObject(m_PtrObj);
	}
...
//		body.m_IsDrawActive = true;
	...
	void BcBoneNormalChara::OnDraw() {
		auto PtrGameStage = GetStage<GameStage>();
		auto shptr = PtrGameStage->FindTagGameObject<BcPNTnTBoneModelRenderer>(L"BcPNTnTBoneModelRenderer");
		shptr->AddDrawObject(m_PtrObj);
	}
