...
namespace basecross{
	...
	void Player::CreateBuffers(){
		...
		//頂点バッファの作成（変更できない）
		MeshResource::CreateVertexBuffer(m_VertexBuffer, vertices);
		//頂点数の設定
		m_NumVertices = static_cast<UINT>(vertices.size());
		//インデックスバッファの作成
		MeshResource::CreateIndexBuffer(m_IndexBuffer, indices);
		//インデックスの設定
	...
	void Player::OnCreate(){
		...
		DrawComp->SetVertexBuffer(m_VertexBuffer);
		DrawComp->SetIndexBuffer(m_IndexBuffer);
		DrawComp->SetNumVertices(m_NumVertices);
		DrawComp->SetNumIndicis(m_NumIndicis);
