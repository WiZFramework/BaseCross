...
namespace basecross{
	...
	public:
		...
		*/
		...
		void SetMeshResource(const shared_ptr<MeshResource>& Mesh) {
			m_VertexBuffer = Mesh->GetVertexBuffer();
			m_IndexBuffer = Mesh->GetIndexBuffer();
			m_NumVertices = Mesh->GetNumVertices();
			m_NumIndicis = Mesh->GetNumIndicis();
		}

		void SetVertexBuffer(const ComPtr<ID3D11Buffer>& Buff){
			m_VertexBuffer = Buff;
		}

		void SetIndexBuffer(const ComPtr<ID3D11Buffer>& Buff){
			m_IndexBuffer = Buff;
		}

		void SetNumVertices(UINT u){
			m_NumVertices = u;
		}
		void SetNumIndicis(UINT u){
			m_NumIndicis = u;
		}
		void SetTextureResource(const shared_ptr<TextureResource>& tex){
